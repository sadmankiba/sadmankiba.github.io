...
(function($) {
    ...
    function scrollToAnchor(target) {
        ...
        if ($(target).length) {
            ...
            $('html, body').animate({ scrollTop: elementOffset }, 600, function() { $('body').removeClass('scrolling'); });
        } else { console.debug('Cannot scroll to target `#' + target + '`. ID not found!'); }
    }
    ...
    $(document).on('click', '.navbar-collapse.show', function(e) { let targetElement = $(e.target).is('a') ? $(e.target) : $(e.target).parent(); if (targetElement.is('a') && targetElement.attr('class') != 'dropdown-toggle') { $(this).collapse('hide'); } });
    let pubFilters = {};
    let searchRegex;
    let filterValues;
    let $grid_pubs = $('#container-publications');
    $grid_pubs.isotope({ itemSelector: '.isotope-item', percentPosition: true, masonry: { columnWidth: '.grid-sizer' }, filter: function() { let $this = $(this); let searchResults = searchRegex ? $this.text().match(searchRegex) : true; let filterResults = filterValues ? $this.is(filterValues) : true; return searchResults && filterResults; } });
    ...
    $('.pub-filters').on('change', function() {
        ...
        $grid_pubs.isotope();
        if (filterGroup == "pubtype") { let url = $(this).val(); if (url.substr(0, 9) == '.pubtype-') { window.location.hash = url.substr(9); } else { window.location.hash = ''; } }
    });

    function filter_publications() {
        let urlHash = window.location.hash.replace('#', '');
        let filterValue = '*';
        if (urlHash != '' && !isNaN(urlHash)) { filterValue = '.pubtype-' + urlHash; }
    ...
    function initMap() {
        if ($('#map').length) {
            ...
            if (map_provider == 1) {
                let map = new GMaps({ div: '#map', lat: lat, lng: lng, zoom: zoom, zoomControl: true, zoomControlOpt: { style: 'SMALL', position: 'TOP_LEFT' }, panControl: false, streetViewControl: false, mapTypeControl: false, overviewMapControl: false, scrollwheel: true, draggable: true });
                map.addMarker({
                    lat: lat,
                    lng: lng,
                    click: function(e) {
    ...
    function toggleSearchDialog() {
        if ($('body').hasClass('searching')) {
            ...
            if (!$('#fancybox-style-noscroll').length && document.body.scrollHeight > window.innerHeight) {
                $('head').append('<style id="fancybox-style-noscroll">.compensate-for-scrollbar{margin-right:' +
    ...
    function changeThemeModeClick() {
        ...
        switch (currentThemeMode) {
            case 0:
                ...
                //     if (window.matchMedia('(prefers-color-scheme: dark)').matches) { isDarkTheme = 1; } else if (window.matchMedia('(prefers-color-scheme: light)').matches) { isDarkTheme = 0; } else { isDarkTheme = isSiteThemeDark; }
    ...
    function getThemeVariation() {
        if (!canChangeTheme()) { return isSiteThemeDark; }
        let currentThemeMode = getThemeMode();
        let isDarkTheme;
        switch (currentThemeMode) {
            case 0:
    ...
    function renderThemeVariation(isDarkTheme, init = false) {
        ...
        if (!init) { if ((isDarkTheme === 0 && !$('body').hasClass('dark')) || (isDarkTheme === 1 && $('body').hasClass('dark'))) { return; } }
        if (isDarkTheme === 0) {
            if (!init) { $('body').css({ opacity: 0, visibility: 'visible' }).animate({ opacity: 1 }, 500); }
            $('body').removeClass('dark');
            if (codeHlEnabled) {
                codeHlLight.disabled = false;
                codeHlDark.disabled = true;
            }
            if (diagramEnabled) { if (init) { mermaid.initialize({ theme: 'default', securityLevel: 'loose' }); } else { location.reload(); } }
    ...
    function fixMermaid() {
        ...
        for (let i = 0; i < mermaids.length; i++) {
            ...
            $(mermaids[i]).replaceWith(function() { return $("<div />").append($(this).contents()).addClass('mermaid'); });
        }
    }
    $(document).ready(function() {
        fixHugoOutput();
        fixMermaid();
        if (code_highlighting) { hljs.initHighlighting(); }
        initThemeVariation();
    ...
    $(window).on('load', function() {
        $('.projects-container').each(function(index, container) {
            ...
            let layout;
            if ($section.find('.isotope').hasClass('js-layout-row')) { layout = 'fitRows'; } else { layout = 'masonry'; }
            $container.imagesLoaded(function() {
                $container.isotope({ itemSelector: '.isotope-item', layoutMode: layout, masonry: { gutter: 20 }, filter: $section.find('.default-project-filter').text() });
        ...
        $('.back-to-top').click(function(event) {
            event.preventDefault();
            $('html, body').animate({ 'scrollTop': 0 }, 800, function() { window.location.hash = ""; });
        });
        $('.js-cite-modal').click(function(e) {
            ...
            modal.find('.modal-body code').load(filename, function(response, status, xhr) {
                ...
                } else { $('.js-download-cite').attr('href', filename); }
            });
            modal.modal('show');
        });
        $('.js-copy-cite').click(function(e) {
            ...
            try { document.execCommand('copy'); } catch (e) { console.log('Error: citation copy failed.'); }
        ...
        $(document).on('keydown', function(e) {
            if (e.which == 27) { if ($('body').hasClass('searching')) { toggleSearchDialog(); } } else if (e.which == 191 && e.shiftKey == false && !$('input,textarea').is(':focus')) {
                e.preventDefault();
                toggleSearchDialog();
            }
        });
    ...
    $('body').on('mouseenter mouseleave', '.dropdown', function(e) {
        ...
        fixScrollspy();
        if (window.location.hash) { if (window.location.hash == "#top") { window.location.hash = "" } else if (!$('.projects-container').length) { scrollToAnchor(); } }
        let resizeTimer;
